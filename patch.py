...
def patch_gradio_demo():
    filepath = "app/scripts/run_gradio_demo.py"
    if not os.path.exists(filepath):
        print(f"Skipping patch: {filepath} not found.")
        return

    # We overwrite the gradio UI to ensure stability, proper threading, and native webcam streaming
    with open(filepath, "w", encoding="utf-8") as f:
        f.write('''# -- triton_key monkey-patch ----------------------------------------------------
...
def on_generate_click(mode, video_path, image):
    if mode == "image to image":
        ...
        _, processed = process_frame(to_bgr(image))
        return to_rgb(processed)
...
if __name__ == "__main__":
    main()''')
    print(f"Patched {filepath} using clean native Gradio streaming implementation.")

def patch_inference_subprocess():
    import os
    filepath = "app/src/fluxrt/stream_processor/model_inference_subprocess.py"
    if not os.path.exists(filepath):
        print(f"Skipping patch: {filepath} not found.")
        return
    ...
    old_ref = """        if self.config.get("use_reference_image", False):
    ...
    new_ref = """        if self.config.get("use_reference_image", False):
            ...
            if image is None:
                ...
                print(
                    f"Warning: use_reference_image is set to true but no valid reference_image_path is provided at '{path}'. You can upload a reference image in the UI."
                )
            else:
                image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
                image = cv2.resize(image, (resolution["width"], resolution["height"]))
    ...
    if "no valid reference_image_path is provided at" not in content:
